# Hi 👋, I'm Katikam Nethaji

⚡ Electronics & Communication Engineer  
🔌 IoT • Embedded Systems • PCB Design  
🚀 Turning circuits into real-world solutions

---

🌸 About Me

- 🎓 B.Tech in Electronics & Communication Engineering  
- 🏫 Brilliant College of Engineering, Hyderabad  
- 🔧 Hands-on experience in **PCB design, circuit debugging & embedded systems**  
- 🏆 State & National Level **India Skills Competition (Electronics)** participant  
- 💡 Passionate about building **IoT, automation, and smart hardware systems**

---

## 🎓 Education
- **B.Tech – ECE**  
  Brilliant College of Engineering, Hyderabad  
  _Pursuing (2029)_

- **CTS – TESDR (NSQF Level-4)**  
  National Skill Training Institute, Hyderabad  
  **GPA: 9.0**

- **Intermediate (MPC)** – JNV Nalgonda  
  **GPA: 6.7**

- **SSC** – ZPP High School, Nakrekal  
  **GPA: 10.0**

---

## 🏆 Achievements & Certifications
- 🥇 **State-Level Winner – India Skills Competition (Electronics), 2024**
- 🏅 Represented **Telangana at National-Level India Skills Competition**
- 🥇 **Trade First Ranker** – TESDR at NSTI

---

## 🧑‍💻 Internship
**PCB Designer – ITSAGE**  
📅 May 2025 – Present  
- Designing and troubleshooting PCB layouts  
- Working with real-world electronic systems and tools

  
## 🧑‍💻 Freelance
**Embedded Trainer – ITSAGE** 
📅 December 2025 – Present
-Teaching the Basic Electronics Concepts
-Training & Mentoring b.tech final year students on embedded c,projects

---

## 🚀 Projects

### 🔹 Hand Gesture Controlled LEDs
- Controlled LEDs wirelessly using **hand gestures**
- Technologies: **MediaPipe + ESP8266**
- Focus on real-time gesture detection & IoT communication

### 🔹 Home Automation System
- Controlled home appliances via **Wi-Fi**
- Used **Arduino / ESP modules + Relay**
- Smartphone-based automation

### 🔹 Line Follower Robot
- Autonomous robot using **IR sensors & Arduino**
- Demonstrates robotics & sensor-based navigation

---

## 🛠 Technical Skills

### 🔧 Electronics & Hardware
- Circuit Design, Soldering, PCB Design
- Multimeter, Oscilloscope, Debugging

### 🤖 Microcontrollers
- Arduino, ESP8266, ESP32
- Raspberry Pi, NodeMCU

### 🧪 Simulation & Design Tools
- Proteus, Tinkercad
- LTSpice, KiCad, Eagle

### 💻 Programming
- C Language
- Embedded C
- Python (Basics)

---

## 🤝 Soft Skills
- Teamwork & Collaboration
- Time Management
- Adaptability & Quick Learning
- Leadership & Project Management

---

## 🌐 Languages
- English
- Telugu
- Hindi

---

## 📫 Connect With Me
- 📧 Email: **nethajigoud26@gmail.com**
- 🔗 LinkedIn: [linkedin.com/in/nethaji-goud-26rn](https://linkedin.com/in/nethaji-goud-26rn)

---
📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Net-is-funny26&show_icons=true&theme=tokyonight)


